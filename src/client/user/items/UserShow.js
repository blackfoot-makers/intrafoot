...
class UserShow extends PureComponent {
  ...
  render() {
    ...
    if (!user) {
      return <Cell col={12}>Contact introuvable</Cell>;
    }
    ...
    return (
      <List>
        ...
        {contact &&
          <ListItem>
            Interlocuteur:{' '}
            <Link to={`/contact/${user.interlocuteur}`}>
              {contact.firstName}
            </Link>
          </ListItem>}
        {user.lastContact &&
          <ListItem>
            Dernier contact:{' '}
            <span>{moment(user.lastContact).format('LL')}</span>
          </ListItem>}
        {user.projects &&
          <ListItem>
            Projets en commun:
        ...
        <ListItem>
          {"Facilité d'accès: "}
          {Array.from({ length: user.accessLevel }, (elem, key) =>
            <Icon name="star" key={key} />
          )}
          {Array.from({ length: 5 - user.accessLevel }, (elem, key) =>
            <Icon name="star_border" key={key} />
          )}
        </ListItem>
        <ListItem>
          Autre sites:
